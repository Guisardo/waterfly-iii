...
class OfflineModeBanner extends StatefulWidget {
  ...
  const OfflineModeBanner({super.key, this.onLearnMore});
...
class _OfflineModeBannerState extends State<OfflineModeBanner> {
  ...
  Future<void> _loadDismissalState() async {
    try {
      ...
      _logger.warning('Failed to load banner dismissal state', e, stackTrace);
      if (mounted) {
        setState(() {
          _isLoading = false;
        });
  ...
  Future<void> _saveDismissalState(bool dismissed) async {
    try {
      ...
      _logger.warning('Failed to save banner dismissal state', e, stackTrace);
    }
  }
