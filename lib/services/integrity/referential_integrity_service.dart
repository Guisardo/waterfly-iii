...
class ReferentialIntegrityService {
  ReferentialIntegrityService({required AppDatabase database}) : _database = database;
  ...
  Future<bool> canDeleteAccount(String accountId) async {
    try {
      ...
      final int transactionCount = await (_database.select(_database.transactions)
      ...
      _logger.info('Account $accountId can delete: $canDelete (transactions: $transactionCount)');
      return canDelete;
    } catch (error, stackTrace) {
      _logger.severe('Failed to check account deletion: $accountId', error, stackTrace);
  ...
  Future<bool> canDeleteCategory(String categoryId) async {
    try {
      ...
      final bool canDelete = transactionCount == 0;
      _logger.info('Category $categoryId can delete: $canDelete (transactions: $transactionCount)');
      return canDelete;
    } catch (error, stackTrace) {
      _logger.severe('Failed to check category deletion: $categoryId', error, stackTrace);
  ...
  Future<bool> canDeleteBudget(String budgetId) async {
    try {
      ...
      final int transactionCount = await (_database.select(_database.transactions)
            ..where(($TransactionsTable t) => t.budgetId.equals(budgetId)))
          .get()
          .then((List<TransactionEntity> list) => list.length);

      final bool canDelete = transactionCount == 0;
      _logger.info('Budget $budgetId can delete: $canDelete (transactions: $transactionCount)');
      return canDelete;
    } catch (error, stackTrace) {
      _logger.severe('Failed to check budget deletion: $budgetId', error, stackTrace);
  ...
  Future<void> cascadeDeleteAccount(String accountId) async {
    try {
      ...
      await _database.transaction(() async {
        ...
        // Delete the account
        await (_database.delete(_database.accounts)..where(($AccountsTable a) => a.id.equals(accountId))).go();
      });

      _logger.info('Cascade delete completed for account: $accountId');
    } catch (error, stackTrace) {
      _logger.severe('Failed to cascade delete account: $accountId', error, stackTrace);
  ...
  Future<void> cascadeDeleteCategory(String categoryId) async {
    try {
      ...
      await _database.transaction(() async {
        ...
        // Delete the category
        await (_database.delete(_database.categories)..where(($CategoriesTable c) => c.id.equals(categoryId))).go();
      });

      _logger.info('Cascade delete completed for category: $categoryId');
    } catch (error, stackTrace) {
      _logger.severe('Failed to cascade delete category: $categoryId', error, stackTrace);
  ...
  Future<List<String>> findOrphanedTransactions() async {
    try {
      ...
      final List<TransactionEntity> allTransactions = await _database.select(_database.transactions).get();
      final List<AccountEntity> allAccounts = await _database.select(_database.accounts).get();
      final List<CategoryEntity> allCategories = await _database.select(_database.categories).get();

      final Set<String> accountIds = allAccounts.map((AccountEntity a) => a.id).toSet();
      final Set<String> categoryIds = allCategories.map((CategoryEntity c) => c.id).toSet();

      final List<String> orphanedIds = <String>[];

      for (final TransactionEntity txn in allTransactions) {
        if (!accountIds.contains(txn.sourceAccountId) || !accountIds.contains(txn.destinationAccountId)) {
          orphanedIds.add(txn.id);
        } else if (txn.categoryId != null && !categoryIds.contains(txn.categoryId)) {
          orphanedIds.add(txn.id);
        }
      }
  ...
  Future<int> repairOrphanedTransactions() async {
    try {
      ...
      for (final String id in orphanedIds) {
        await (_database.delete(_database.transactions)..where(($TransactionsTable t) => t.id.equals(id))).go();
      }

      _logger.info('Repaired ${orphanedIds.length} orphaned transactions');
      return orphanedIds.length;
    } catch (error, stackTrace) {
      _logger.severe('Failed to repair orphaned transactions', error, stackTrace);
  ...
  Future<Map<String, int>> performIntegrityCheck() async {
    try {
      ...
      final List<String> orphanedTransactions = await findOrphanedTransactions();
      ...
      results['invalid_accounts'] = accounts.where((AccountEntity a) => a.name.isEmpty).length;

      // Check for categories with invalid data
      final List<CategoryEntity> categories = await _database.select(_database.categories).get();
      results['invalid_categories'] = categories.where((CategoryEntity c) => c.name.isEmpty).length;
  ...
  Future<Map<String, int>> repairAllIssues() async {
    try {
      ...
      results['orphaned_transactions_repaired'] = await repairOrphanedTransactions();

      // Remove invalid accounts
      final List<AccountEntity> accounts = await _database.select(_database.accounts).get();
      for (final AccountEntity account in accounts.where((AccountEntity a) => a.name.isEmpty)) {
        await (_database.delete(_database.accounts)..where(($AccountsTable a) => a.id.equals(account.id))).go();
        results['invalid_accounts_removed'] = results['invalid_accounts_removed']! + 1;
      }

      // Remove invalid categories
      final List<CategoryEntity> categories = await _database.select(_database.categories).get();
      for (final CategoryEntity category in categories.where((CategoryEntity c) => c.name.isEmpty)) {
        await (_database.delete(_database.categories)..where(($CategoriesTable c) => c.id.equals(category.id))).go();
        results['invalid_categories_removed'] = results['invalid_categories_removed']! + 1;
      }
