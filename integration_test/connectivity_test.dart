...
void main() {
  ...
  group('Connectivity Detection Tests', () {
    testWidgets('Detect offline state when connectivity is disabled', (
      WidgetTester tester,
    ) async {
      app.main();
      ...
      final Finder connectivityBar = find.byKey(
        const Key('connectivityStatusBar'),
      );

      // Check if offline mode is detected
      expect(
        offlineIndicator.evaluate().isNotEmpty ||
            connectivityBar.evaluate().isNotEmpty,
        isTrue,
        reason:
            'App should show offline indicator when connectivity is disabled',
      );
    });

    testWidgets('Detect online state when connectivity is enabled', (
      WidgetTester tester,
    ) async {
      app.main();
      await tester.pumpAndSettle();
      ...
      expect(
        offlineIndicator.evaluate().isEmpty ||
            onlineIndicator.evaluate().isNotEmpty,
        isTrue,
        reason: 'App should not show offline indicator when online',
      );
    });

    testWidgets('React to connectivity changes during runtime', (
      WidgetTester tester,
    ) async {
      app.main();
      await tester.pumpAndSettle();
